...
**Statement.** The paper now isolates a closed core entropy-drift theorem: if the active self-reinforcing repertoire exerts non-decreasing net entropy-reducing pressure, then conditional macrostate entropy contracts. The missing step has been narrowed twice: `proofs/maintenance_lemma.md` first supplies a sufficient balance lemma showing that load grows when newly stabilized mechanisms compensate for losses, and that net pressure is maintained when incoming pressure, survivor strengthening, and coherent-excess change compensate for shed pressure; it now also proves this balance for deficit-responsive renewal systems, where lost entropy-contracting mechanisms trigger replacement mechanisms whose expected incoming pressure, together with survivor/coherent increment, dominates expected shed pressure. What remains is to classify which ACP systems have renewal-dominant deficit-response channels, and to derive analogous maintenance conditions in further target dynamics.
**Status.** Partial — first model-class closure for renewal-dominant birth-death / replacement systems.
...
**Why it matters.** This is still the sharpest remaining gap in the full four-part CDT package. The bookkeeping step is explicit, and one renewal-dominant model class now satisfies it, but the load / basin / asymptotic claims remain conditional outside classes where the maintenance-balance inequality can be derived in the target dynamics.

### OP-17 — Generic k-mechanism closure under intersection
**Statement.** Appendix A.9 now states explicitly what earlier drafts used implicitly: the generic k-mechanism induction needs an intersection-compatibility hypothesis for compound basins and a threshold ensuring the combined return channels dominate the exterior return probability to the intersection. The first non-Gaussian model-class closure is now written: for aligned positive-association kernels, positive return association plus a product-overlap threshold makes the compound basin self-reinforcing, and monotone / MTP2 systems give a checkable sufficient form. What remains is to classify which ACP / CDT systems generate this alignment internally, which require it as a domain hypothesis, and which fail intersection closure because their active basins are antagonistic, non-comparable, or only weakly overlapping.
**Status.** Partial — first structured model-class closure for aligned positive-association / monotone MTP2 kernels.
**Where discussed.** `proofs/induction_step_k_mechanisms.md`; `audits/proof_debt_v11.md`.
**Why it matters.** The Gaussian induction is clean, and the monotone positive-association class now gives a first non-Gaussian positive template, but the generic induction is only as strong as the closure lemma carrying the compound basin forward. Classifying the positive and failure classes would remove the most serious remaining overreach in the k > 2 story.
...
### OP-5 — Schur bridge: adaptive coherence steering as rank restoration
**Statement.** *Adaptive coherence steering mechanisms ↔ operations that restore D's rank / condition number.* Characterize which operations in the mechanism algebra achieve this — i.e., produce a formal bijection or at least a structural correspondence between "noise injection," "environmental perturbation," and "regularization" on one side, and rank-restoring operations on the Schur side.
**Status.** Open.
**Where discussed.** `bridges/schur_complement.md`.

### OP-7 — Coordination neutrality under tree composition
**Statement.** Two-argument coordination-neutral operators B(x,y) satisfy B(y,x) = 1/B(x,y). Log-lift L(x,y) = log|B(x,y)| is swap-antisymmetric. The bridge family (exp–log operators, e.g. Odrzywolek's eml(x,y) = exp(x) − ln(y)) is CN pairwise. Under tree composition required for multi-party coordination, full leaf-reversal CN requires joint-inversion invariance, but the new three-stroke bridge shows that CN does survive under hierarchical block swaps as wreath symmetry. **Characterize which operator families, or which operator cycles, preserve useful CN-derived symmetry under recursive composition.**
**Status.** Partial — `bridges/three_stroke_persistence_engine.md` resolves the immediate all-or-nothing version by distinguishing block-swap wreath symmetry from full leaf-reversal CN. The full classification remains open.
**Where discussed.** `bridges/coordination_neutrality.md`; `bridges/three_stroke_persistence_engine.md`; external reference `references/odrzywolek_2026_eml_operator.pdf`.
**Why it matters.** Multi-party coordination may not require global CN in the naive reversal sense, but it does require a stable compositional symmetry. The three-stroke bridge suggests the object to classify is not a lone CN operator but a coordination-generation-steering cycle.

### OP-21 — Three-stroke operator universality and reproducibility
**Statement.** The new operator bridge proposes a minimal ACP cycle \(C \to G \to N_\varepsilon\): coordination-neutral self-seeding, EML-class generation, and CN-invariant antisymmetric adaptive-coherence steering. The formal local analysis is written, but the reported plateau simulations are not yet reproduced in-repo, and the universality question is open: can a CN-centered cycle be both elementary-function generative and naturally stable under recursive depth?
**Status.** Open / partial. The local operator algebra and gate threshold are written in `bridges/three_stroke_persistence_engine.md`; simulation reproducibility, global stability, ternary compression, and universality within persistence remain open.
**Where discussed.** `bridges/three_stroke_persistence_engine.md`; `bridges/coordination_neutrality.md`; `bridges/valuation_cocycle_bridge.md`.
**Why it matters.** This is the cleanest current route from ACP as a dynamical principle to ACP as an operator algebra. Closing it would produce a constructive primitive for persistence, not only a condition on already-given dynamics.
**Adjacent to.** OP-7 (CN under tree composition), OP-15 (arithmetic shadows), OP-20 (control-theory continuation variety), and A.21 generativity.

### OP-22 — Vacuum constitutive bridge for reciprocal coordination geometry
**Statement.** The reciprocal coordination geometry bridge derives a structural chain from CN reciprocity to Lorentz cone geometry, a local \(U(1)\) phase bundle, Maxwell curvature, and an effective propagation speed \(c_{\mathrm{eff}}=\xi/\tau\). In media it matches the slope to \(c_{\mathrm{eff}}=1/\sqrt{\mu\epsilon}\), but the constitutive parameters are not yet derived from CN first principles. Derive the vacuum relations \(D=\epsilon_0 E\), \(B=\mu_0 H\), or their CN-native analogues, from operator parameters such as \((\alpha_0,\lambda_0)\) rather than importing \(\epsilon_0\) and \(\mu_0\) by electromagnetic calibration.
**Status.** Open / decomposed. The Lorentz / \(U(1)\) / Maxwell structural derivation is written in `bridges/reciprocal_coordination_geometry.md`; `audits/reciprocal_coordination_geometry_audit.md` decomposes the remaining work into CN medium state, compliance / stiffness from second variations, wave-speed theorem, electromagnetic identification, and vacuum normalization.
**Where discussed.** `bridges/reciprocal_coordination_geometry.md`; `audits/reciprocal_coordination_geometry_audit.md`; `bridges/three_stroke_persistence_engine.md`; `bridges/coordination_neutrality.md`.
**Why it matters.** Without this step, the bridge explains why CN phase transport has Lorentz-Maxwell form but still treats measured light speed through a matching condition. Closing it would turn the geometric bridge from structural recovery into a stronger first-principles route to electromagnetic constants.
**Adjacent to.** OP-21 (operator-level universality), OP-RP-5 in `bridges/restraint_power.md` (operator-algebra extension), and the possible quantum ACP bridge from CN phase to probability.
...
### OP-20 — Control-theory continuation variety bridge
**Statement.** Promote the standalone control-theory reduction from structural bridge to quantitative theorem. The current note identifies the information-conditioned viable continuation count $N_h(m_t)$ as the control-native proxy for conditional macrostate entropy, uses data-rate-limited stabilization as the dissolution floor, and uses saturation-locked regulation as the crystallization boundary. What remains is to quantify this bridge for nonlinear / continuous-time plants, distinguish ordinary exact tracking from genuine crystallization by a sharp task-neighborhood criterion, formalize decentralized visibility through action-as-signal problems, and derive a control-native maintenance condition for adaptive, gain-scheduled, or learned controllers.
**Status.** Partial. The structural ACP control register is written in `reductions/control_theory.md`; the quantitative, decentralized, and adaptive-controller CDT extensions remain open.
**Where discussed.** `reductions/control_theory.md`; `paper/acp_main_v10.md` §4.4.6 and §7.10.
**Why it matters.** Control theory is the most direct engineered domain for testing whether persistence requires preserved maneuver authority rather than stability alone. Closing this bridge would turn anti-windup, robust-yet-fragile loop shaping, and adaptive-controller aging into a publishable control-native ACP testbed.
**Adjacent to.** OP-16 (maintenance lemma), OP-18 (visibility / kernel-preserving transformation), Prediction 8 (dissipative aging), Prediction 9 (regulatory-network aging), and possible Witsenhausen-style restraint-power formalization.
