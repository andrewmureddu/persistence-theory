...
The algebraic paper's Section 4 identifies the mathematical ingredients for deriving the Heisenberg uncertainty principle from persistence requirements. With the Schur complement bridge formalized, this program can be restated more precisely: the claim is that the requirement rank(D) > 0 (persistence requires non-trivial internal structure) forces the existence of conjugate pairs of variables satisfying [Q, P] = iκI with κ > 0. The chain:

1. rank(D) > 0 → internal degrees of freedom exist → the observable algebra is non-commutative (Tomita-Takesaki)
...
The gap remains: step 1 requires showing that rank(D) > 0 in a quantum system forces non-commutativity of the associated operator algebra. This is plausible (a system with non-trivial internal stochastic dynamics should have incompatible observables) but unproven.
