...
- `bee_follower_null_analysis.py`: follower-channel null diagnostics for the bee coupling result. It adds rarefied follower entropy / concentration and follower-identity shuffles that keep later-window event counts fixed, optionally conditioning the shuffle by day, feeder, dancer, or dancer-feeder event slots.
- `bee_follower_strata_diagnostics.py`: composition-driver diagnostics for the bee follower nulls. It ranks day, day+feeder, day+dancer, and day+dancer+feeder strata by how strongly their later-window event shares align with current-window recruitment predictors.
...
The first follower-null pass sharpens the caveat. A stricter 50-event rarefaction still preserves the direction of the one-hour signal (`follower_events_per_dance -> follower_entropy` rarefied Spearman about `0.58`; `follower_events_per_dance -> dominant_follower_fraction` rarefied Spearman about `-0.67`, `n = 24`). However, follower-identity shuffles that preserve later-window event counts produce null correlations at least as strong as the observed ones. Conditioning those shuffles by day, day+feeder, day+dancer, or day+dancer+feeder does not rescue the result.

The stratum-driver diagnostic makes the artifact concrete. Among event-rich strata, current `follower_events_per_dance` is already strongly aligned with later pooled follower diversity and anti-aligned with later pooled follower dominance: day-level Spearman correlations are about `0.78` and `-0.85`; day+feeder correlations are about `0.74` and `-0.73`. The current bee signal should therefore be treated as a day / feeder / dancer composition artifact until a stronger event-level or mixed-model analysis says otherwise.
