...
**10. Social-insect empirical track.** A new Tier-2 observational program now exists at `bridges/social_insect_empirical_program.md` and `empirical/social_insects/`. It targets existing ant and honey-bee datasets as publishable empirical tests of ACP observables: perturbation absorption, spatial / interaction entropy reallocation, waggle-dance communication diversity, follower-network concentration, and productive foraging partitioning. First-pass Berlin 2019 honey-bee files from Zenodo `10.5281/zenodo.7928121` are downloaded locally (ignored by git), with scripts for waggle entropy, contiguous-lag dance/follower coupling, controlled lag diagnostics, follower-sample nulls, and stratum-driver diagnostics. The corrected contiguous-lag result is nontrivial: high follower activity per dance predicts higher follower entropy and lower dominant-follower concentration one hour later, suggesting successful recruitment may reopen the follower channel rather than simply crystallizing it. Basic sampling-effort controls attenuate the signal; 50-event rarefaction preserves the directional result, but identity-shuffle nulls that keep later-window event counts fixed produce null correlations at least as strong as the observed ones. New targeted shuffles conditioned by day, day+feeder, day+dancer, and day+dancer+feeder also fail to rescue the recruitment-specific reading. The stratum-driver diagnostic makes the confound concrete: across event-rich day and day+feeder strata, current follower activity is already strongly aligned with later pooled follower diversity and anti-aligned with later dominant-follower fraction. The current bee result is therefore best treated as a day / feeder / dancer composition artifact unless a stronger event-level or mixed model says otherwise. Dryad terminal download for the ant homeostasis dataset still returns 403, but the authors' companion GitHub repository supplies trophallaxis spreadsheets; the first ant pass finds low-density interaction-location entropy rising in all three colonies, with interaction throughput rising in colonies 1-2 and falling modestly in colony 3. *(Priority: high — the ant perturbation track now looks cleaner than the current bee follower-channel track.)*
...
### 2026-05-04 — bee follower conditioned nulls
- Extended `empirical/social_insects/bee_follower_null_analysis.py` so follower-identity shuffles can condition on day, day+feeder, day+dancer, or day+dancer+feeder event slots while preserving later-window follower-event counts.
- Ran targeted conditioned-null checks for the two headline predictors and two follower-channel outcomes. The rarefied directional signal remains, but identity-shuffle p-values stay high under all conditioning schemes, including `day-feeder` and `day-dancer-feeder`.
- Added `empirical/social_insects/bee_follower_strata_diagnostics.py` and ran the 60m / one-lag / 50-event stratum-driver check. Event-rich day, day+feeder, day+dancer, and day+dancer+feeder strata already align current follower activity with later follower-pool diversity, explaining why the conditioned nulls reproduce the headline direction.
- Updated `empirical/social_insects/README.md` and `empirical/social_insects/first_pass_results_2026-05-01.md` to mark the current bee follower result as likely day / feeder / dancer composition rather than controlled recruitment-specific identity broadening.
- Session log: `sessions/2026-05-04_social_insect_conditioned_nulls.md`.
...
- Added `empirical/social_insects/bee_waggle_controlled_lag_analysis.py` for residualized lag diagnostics, `empirical/social_insects/bee_follower_null_analysis.py` for rarefaction / identity-shuffle follower nulls, and `empirical/social_insects/ant_trophallaxis_summary.py` for the Modlmeier companion trophallaxis spreadsheets.
...
- Updated `empirical/social_insects/first_pass_results_2026-05-01.md` with the corrected bee contiguous-lag result, controlled caveat, follower-null caveat, and ant trophallaxis density contrasts.
