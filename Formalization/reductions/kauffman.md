...
(c) The frozen component fraction f is the Kauffman-specific analog of the ACP’s compound reinforcement-basin occupancy. Each frozen node is a node whose state is fixed by the accumulated self-reinforcing mechanisms (the stable Boolean functions and wiring patterns that lock it into a constant value). As additional freezing constraints accumulate, the admissible macrostate set R̅(t) shrinks in set-inclusion while the node-wise footprint of those constraints expands. In that sense, frozen-component expansion is the observable signature of a shrinking compound reinforcement basin.
...
(d) **Frozen component expansion is the Boolean-network signature of crystallization drift. **Under the maintained-pressure and load-growth hypotheses behind the CDT’s repertoire-geometry layer, the expansion of the frozen component under selection pressure—Kauffman’s key computational observation—is the Boolean-network manifestation of crystallization drift. Each frozen node is a self-reinforcing mechanism (its fixed state is stabilized by its Boolean function and inputs). As frozen nodes accumulate, the admissible macrostate set R̅ shrinks while H(m′ | m) monotonically decreases. The frozen component’s expansion is not an accidental feature of selection on Boolean networks but the domain-specific signature of the CDT operating on network-level self-reinforcing mechanisms.
...
(d) Each frozen node i satisfies the self-reinforcement condition (Definition 4.7): the probability that node i remains in its fixed state at t+1, given that it is in that state at t, exceeds the probability of it entering that state from outside. This is guaranteed by the definition of freezing—a frozen node’s Boolean function and inputs lock it into a constant value across the entire attractor. The compound reinforcement basin R̅ = ∩ᵢ Rᵢ is the set of macrostates where all currently frozen nodes remain frozen. Under the CDT’s conditional repertoire-geometry layer, R̅ can only shrink (or stay constant) in set-inclusion as new freezing constraints are added, while the number of frozen nodes expands. The observable growth of the frozen component is therefore the node-level signature of a shrinking admissible macrostate set.
...
***Proposition A.15.5 (CDT as frozen component expansion). ***Under the variable identification of Definition A.15.1, and under the maintained-pressure/load-growth hypotheses behind the CDT’s repertoire-geometry layer, the Crystallization Drift Theorem (Theorem 4.19) makes the following specific predictions for Boolean networks under selection:
